...
import main_28_days_cancellation
# import asyncio
...
#         main_28_days_cancellation.NHSPRequirementsCompare.comparing()
...
@scheduler.scheduled_job(trigger=IntervalTrigger(minutes=10), id='scraping_28_days')
def task_28_days_scraping():
    scheduler.remove_job('scraping_28_days')
    # logger.info(f'Scrapping task started at: {datetime.now()}')
    
    main_28_days_cancellation.NHSPRequirementsCompare()
    main_28_days_cancellation.NHSPRequirementsCompare.comparing()
                    
    # NHSPPortalScraping.scraping()
    # logger.info(f'Scrapping task ended at: {datetime.now()}')
    scheduler.add_job(task_28_days_scraping, trigger=IntervalTrigger(hours=2), id='scraping_28_days')
 
# @scheduler.scheduled_job(trigger=CronTrigger(hour=11, minute=18), id='delete_files')
@scheduler.scheduled_job(trigger='cron', day_of_week='fri', hour=11, minute=16, id='delete_files')
def task_delete_files():
    ...
    main_28_days_cancellation.ArchiveFiles.delete_files_based_on_date()
