...
import main_3_days_cancellation
...
# def filled_3_days_cancellation():
    
#         print("start Scheduler of Scraping website for 3 days...")
#         # Your scraping logic here
#         main_3_days_cancellation.NHSPRequirementsCompare.comparing()
...
# scheduler.add_job(filled_3_days_cancellation,'interval',minutes=4, id='job_1', next_run_time=datetime.now())
...
@scheduler.scheduled_job(trigger=IntervalTrigger(seconds=30), id='scraping_3_days')
def task_3_days_scraping():
    scheduler.remove_job('scraping_3_days')
    logger.info(f'Scrapping task started at: {datetime.now()}')
    print('inside scraping job')
    main_3_days_cancellation.NHSPRequirementsCompare()
    main_3_days_cancellation.NHSPRequirementsCompare.comparing()
   
    logger.info(f'Scrapping task ended at: {datetime.now()}')
    scheduler.add_job(task_3_days_scraping, trigger=IntervalTrigger(seconds=30), id='scraping_3_days')



# @scheduler.scheduled_job(trigger=CronTrigger(hour=14, minute=47), id='delete_files')
...
#     # main_3_days_cancellation.ArchiveFiles()
#     main_3_days_cancellation.ArchiveFiles.delete_files_based_on_date()

#     logger.info(f'Delete files task ended at: {datetime.now()}')
    
#     # scheduler.add_job(task_delete_files, trigger=CronTrigger(hour=14, minute=26), id='delete_files')
